...
class ProductFactory {
    ...
    static async publishProductByShop({product_shop , product_id}) {
        return await findByProductShopAndIdAndUpdateIsPublish({product_shop,product_id});  
    }
    static async unPublishProductByShop({product_shop , product_id}) {
        return await findByProductShopAndIdAndUpdateUnPublish({product_shop,product_id});  
    }     
    //END PUT // 
    ...
    static async findProduct({product_id}) {
        return await findOneProduct({product_id , unslect: ['__v']})
    }
    //end query//
}
...
class Clothing extends Product {
    async createProduct() {
        ...
        if(!newClothing) {
            throw new BadRequestError('create new clothing fail!');
        }
        const newProduct = await super.createProduct(newClothing._id);
        if(!newProduct) {
            throw new BadRequestError('create new product fail!');
        }
        return newProduct;
    }
    ...
    async updateProduct( productId ) {
        ...
        if(objectParams.product_attributes) {
            await updateProductById({productId , payload:updateNestedObjectParser(objectParams) , model: clothing})
        }
...
class Electronic extends Product {
    async createProduct() {
        ...
        if(!newElectronic) {
            throw new BadRequestError('create new Electronics fail!');
        }
    ...
    async updateProduct( productId ) {
        ...
        if(objectParams.product_attributes) {
            await updateProductById({productId , payload: updateNestedObjectParser(objectParams.product_attributes) , model: electronic})
        }
...
class Furniture extends Product {
    async createProduct() {
        ...
        if(!newFurniture) {
            throw new BadRequestError('create new Furnitures fail!');
        }
